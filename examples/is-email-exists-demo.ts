...
async function basicCheckIfExistsDemo() {
  ...
  for (const email of testEmails) {
    ...
    try {
      ...
      if (result.smtp) {
        console.log(`🔌 SMTP: ${result.smtp.canConnectSmtp ? 'Connected' : 'Failed'}`);
        if (result.smtp.canConnectSmtp) {
          console.log(`   Deliverable: ${result.smtp.isDeliverable ? 'Yes' : 'No'}`);
          console.log(`   Can connect: ${result.smtp.canConnectSmtp ? 'Yes' : 'No'}`);
        }
      }
    } catch (error) {
...
async function customSmtpVerificationDemo() {
  ...
  for (const config of smtpConfigs) {
    ...
    try {
      ...
      console.log(`   Success: ${smtpResult.canConnectSmtp}`);
      console.log(`   Deliverable: ${smtpResult.isDeliverable}`);
      console.log(`   Can connect: ${smtpResult.canConnectSmtp}`);
