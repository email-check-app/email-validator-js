/**
 * Type definitions used by check-if-email-exists.ts.
 *
 * `EmailProvider` is the canonical runtime enum re-exported from ./types.
 * `SmtpVerificationResult` is duplicated below (instead of re-exported)
 * because the consumer imports it without a `type` keyword and bun is
 * stricter than ts-jest about type-only re-exports — a missing runtime
 * binding becomes a hard link error. The two declarations must stay in
 * structural lock-step.
 */

import type { Cache } from '../../src/cache-interface';
import type { EmailProvider } from '../../src/types';

export { EmailProvider } from '../../src/types';
