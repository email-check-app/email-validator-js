...
import { EmailProvider, verifyYahooApi } from '../check-if-email-exists';
import type { YahooApiOptions } from '../email-verifier-types';

// Mock fetch for Yahoo API tests. Cast through `unknown` because the bun
// `Mock<...>` type doesn't structurally include the `preconnect` extension
// that `typeof fetch` requires.
...
describe('Email Provider Constants', () => {
  test('should have correct Yahoo provider enum', () => {
    expect<string>(EmailProvider.yahoo).toBe('yahoo');
  });

  test('should have all required email provider enums', () => {
    expect<string>(EmailProvider.gmail).toBe('gmail');
    expect<string>(EmailProvider.hotmailB2c).toBe('hotmail_b2c');
    expect<string>(EmailProvider.hotmailB2b).toBe('hotmail_b2b');
    expect<string>(EmailProvider.proofpoint).toBe('proofpoint');
    expect<string>(EmailProvider.mimecast).toBe('mimecast');
    expect<string>(EmailProvider.everythingElse).toBe('everything_else');
  });
});
